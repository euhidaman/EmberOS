...
EOF
echo -e "${NC}"
echo -e "${BLUE}Ember-OS Installer v${EMBER_VERSION}${NC}"
echo ""
...
if ! command -v llama-server &> /dev/null; then
    ...
    echo "Qwen2.5-VL model requires standard llama.cpp"
fi
...
echo "  • BitNet (port 38080) handles fast text-only tasks (3-5x faster)"
